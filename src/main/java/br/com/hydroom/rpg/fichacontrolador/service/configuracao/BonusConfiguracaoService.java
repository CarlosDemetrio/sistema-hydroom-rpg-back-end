...
public class BonusConfiguracaoService extends AbstractConfiguracaoService<BonusConfig, BonusConfigRepository> {
    ...
    /**
     * Cria um BonusConfig, preenchendo {@code ordemExibicao} automaticamente
     * como {@code MAX + 1} se o valor não for informado (null ou 0).
     */
    @Override
    @Transactional
    public BonusConfig criar(BonusConfig configuracao) {
        if (configuracao.getOrdemExibicao() == null || configuracao.getOrdemExibicao() == 0) {
            configuracao.setOrdemExibicao(
                calcularProximaOrdemExibicao(configuracao.getJogo().getId(), "BonusConfig"));
        }
