...
public interface ConfiguracaoRacaRepository extends JpaRepository<Raca, Long> {
     ...
     */
    @Query("SELECT r FROM Raca r WHERE r.jogo.id = :jogoId AND r.deletedAt IS NULL ORDER BY r.ordemExibicao")
    List<Raca> findByJogoIdAndAtivoTrueOrderByOrdemExibicao(@Param("jogoId") Long jogoId);
