#!/bin/bash
# =====================================================================
# Rollback para versao anterior da imagem Docker
# =====================================================================
#
...
#
# Este script fica em:
#   - /opt/app/scripts/rollback.sh (VM de producao, via setup-oci.sh)
#   - infra/rollback.sh (repositorio, referencia)
#
set -euo pipefail
...
for i in $(seq 1 6); do
    if curl -sf http://localhost:8081/actuator/health/readiness > /dev/null 2>&1; then
        echo "Rollback para $TAG concluido com sucesso!"
        exit 0
    fi
    echo "Tentativa $i/6..."
    sleep 10
