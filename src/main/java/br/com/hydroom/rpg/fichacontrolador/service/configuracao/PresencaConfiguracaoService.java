...
public class PresencaConfiguracaoService extends AbstractConfiguracaoService<PresencaConfig, PresencaConfigRepository> {
    ...
    /**
     * Cria um PresencaConfig, preenchendo {@code ordemExibicao} automaticamente
     * como {@code MAX + 1} se o valor não for informado (null ou 0).
     */
    @Override
    @Transactional
    public PresencaConfig criar(PresencaConfig configuracao) {
        if (configuracao.getOrdemExibicao() == null || configuracao.getOrdemExibicao() == 0) {
            configuracao.setOrdemExibicao(
                calcularProximaOrdemExibicao(configuracao.getJogo().getId(), "PresencaConfig"));
        }
